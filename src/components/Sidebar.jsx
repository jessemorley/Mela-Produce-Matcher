...
export default function Sidebar({ selectedNav, onSelectNav, matchCount, produceCount, recipeCount }) {
  ...
  return (
    <div className="w-64 shrink-0 bg-slate-900/90 border-r border-slate-800/80 flex flex-col">
      ...
      <div className="px-4 py-3 flex items-center space-x-3 mb-4">
        ...
        <div>
          <h1 className="font-bold text-sm tracking-wide text-white">Sprout</h1>
          <p className="text-xs text-slate-400">Seasonal Matcher</p>
        </div>
      </div>

      <div className="px-2 space-y-1">
        <span className="px-3 text-[10px] font-semibold text-slate-500 uppercase tracking-widest block mb-1">
          Library
        </span>
          ...
          >
            ...
            <span
              className={`text-[10px] px-1.5 py-0.5 rounded-full ${
                selectedNav === key ? "bg-emerald-600 text-white" : "bg-slate-800 text-slate-400"
              }`}
            >
              {count(counts)}
            </span>
